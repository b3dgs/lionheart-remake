 ...
 */
public class HotFireBallPart implements Focusable, PropertiesFeature
{
    /** Id. */
    public static final String ID = Activator.PLUGIN_ID + ".part.properties.hotfireball";
    /** Menu. */
    public static final String MENU = ID + ".menu";

    private Tree tree;
    private HotFireBall hotFireBall;
    ...
    {
        return hotFireBall.getConfig() != null;
    }
    ...
    public void enable()
    {
        if (hotFireBall.getConfig() == null)
        {
            final TreeItem item = new TreeItem(tree, SWT.NONE);
            hotFireBall.setConfig(new HotFireBallConfig());
            item.setText(hotFireBall.getConfig().toString());
        }
    }
    ...
    {
        UtilPart.getMPart(HotFireBallPart.ID).setVisible(true);
        ...
        hotFireBall = (HotFireBall) feature;
        if (hotFireBall.getConfig() == null)
        {
            enable();
        }
        ...
        tree.addMouseListener(new MouseAdapter()
        {
            ...
            public void mouseDoubleClick(MouseEvent event)
            {
                ...
                if (items.length > 0)
                {
                    ...
                    final HotFireBallEditor editor = new HotFireBallEditor(tree, hotFireBall.getConfig());
                    editor.create();
                    editor.openAndWait();
                    editor.getOutput().ifPresent(c ->
                    {
                        hotFireBall.setConfig(c);
